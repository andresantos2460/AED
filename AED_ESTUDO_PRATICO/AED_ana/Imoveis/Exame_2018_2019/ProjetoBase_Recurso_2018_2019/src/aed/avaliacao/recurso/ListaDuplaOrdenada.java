...
public class ListaDuplaOrdenada<T> implements ColecaoIteravelLinearOrdenada<T> {
    ...
    public ListaDuplaOrdenada<T> removerTodos(T elem) {
        //TODO: PERGUNTA 2
        ListaDuplaOrdenada<T> removidos = new ListaDuplaOrdenada<>(criterio);
        No primeiro = getNoPorOrdem(elem);
        if (primeiro.compararElemento(elem) > 0) {
            return removidos;
        }
        No ultimo = primeiro.seguinte;
        while (ultimo.compararElemento(elem) == 0) {
            ultimo = ultimo.seguinte;
        }
        ultimo = ultimo.anterior;
        primeiro.anterior.seguinte = ultimo.seguinte;
        ultimo.seguinte.anterior = primeiro.anterior;
        removidos.base.seguinte = primeiro;
        primeiro.anterior = removidos.base;
        ultimo.seguinte = removidos.base;
        removidos.base.anterior = ultimo;
        return removidos;
    }
    ...
    public abstract class No implements Serializable {
        ...
        public abstract int compararElemento(T elem);
    }
    ...
    public class Base extends No {
        ...
        public int compararElemento(T elem) {
            return 1;
        }
    }
    ...
    public class NoComElemento extends No {
        ...
        public NoComElemento(T elem, No seg) {
            if (elem == null) {
