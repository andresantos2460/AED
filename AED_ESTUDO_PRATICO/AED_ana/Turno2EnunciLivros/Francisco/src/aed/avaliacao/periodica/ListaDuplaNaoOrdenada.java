...
public class ListaDuplaNaoOrdenada<T> implements ColecaoIteravelLinearNaoOrdenada<T> {
    ...
    public class IteradorUltimosTresElementos implements IteradorIteravelDuplo<T> {
        ...
        public IteradorUltimosTresElementos() {
            No aux = base.anterior;
            int count = 0;
            while (aux != base && count < 3) {
                aux = aux.anterior;
                count++;
            }
            this.anteriorAoPrimeiro = aux;
            reiniciar();
        }
        ...
        public T corrente() {
            if (corrente == base) {
                throw new NoSuchElementException("Não existe elemento corrente no iterador!");
            }
        ...
        public T avancar() {
            if (!podeAvancar()) {
                throw new NoSuchElementException("Não é possível avançar além do final da lista!");
            }
        ...
        public T recuar() {
            if (!podeRecuar()) {
                throw new NoSuchElementException("Não é possível recuar além dos 3 últimos elementos!");
            }
