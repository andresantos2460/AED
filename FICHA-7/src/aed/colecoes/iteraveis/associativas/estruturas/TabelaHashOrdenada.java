...
public class TabelaHashOrdenada<C, V> implements ColecaoIteravelAssociativa<C, V> {
    ...
    protected TabelaHashComIncrementoPorHashComReHashOtimizado noPorChave;

    public TabelaHashOrdenada(Comparacao<C> cp, int tamanho) {
        criterio = cp;
        base = new No();
        noPorChave = new TabelaHashComIncrementoPorHashComReHashOtimizado(tamanho);
    }
...
//    }
    ...
    private No getNoPorOrdem(C chave) {
        ...
        if (noPorChave.getNumeroElementos() == 0 ||
                criterio.comparar(base.anterior.elemento.getChave(), chave) < 0) {
            return base;
        }

        cor = base.seguinte;
        while (criterio.comparar(cor.elemento.getChave(), chave) < 0) {
            cor = cor.seguinte;
        }
...
//    }
    ...
    public void inserir(C chave, V valor) {
        ...
        noPorChave.inserir(chave, new No(new Associacao<>(chave, valor), getNoPorOrdem(chave)));
    }

    private V removerNo(No no) {
    ...
    public V removerPorIndice(int indice) {
        return removerNo(noPorChave.remover(getNoPorIndice(indice).elemento.getChave()));
    }
    ...
    public V consultarPorIndice(int indice) {
        return getNoPorIndice(indice).elemento.getValor();
    }
    ...
    public class No implements Serializable {
        ...
        protected No(Associacao<C, V> elem, No seg) {
            if (elem == null) {
    ...
    public class Iterador implements IteradorIteravelDuplo<Associacao<C, V>> {
        ...
        protected Iterador(C chaveInicial, C chaveFinal) {
            ...
            anteriorAoPrimeiro = getNoPorOrdem(chaveInicial).anterior;
            if (criterio.comparar(base.anterior.elemento.getChave(), chaveFinal) <= 0) {
                seguinteAoUltimo = base;
            } else {
                No no = noPorChave.consultar(chaveFinal);
                if (no != null) {
                    seguinteAoUltimo = no.seguinte;
                } else {
                    seguinteAoUltimo = anteriorAoPrimeiro.seguinte;

                    while (criterio.comparar(seguinteAoUltimo.elemento.getChave(), chaveFinal) <= 0) {
    ...
    private class TabelaHashComIncrementoPorHashComReHashOtimizado extends TabelaHashComIncrementoPorHash<C, No> {

        public TabelaHashComIncrementoPorHashComReHashOtimizado(int tamanho) {
            super(tamanho);
        }

        @Override
        protected void reHash() {
            tabela = new Entrada[proximoPrimo(tabela.length * 2)];
            numeroElementos = numeroElementosInativos = 0;
            No no = base.seguinte;
            while (no != base) {
                inserir(no.elemento.getChave(), no);
                no = no.seguinte;
            }
        }
    }
