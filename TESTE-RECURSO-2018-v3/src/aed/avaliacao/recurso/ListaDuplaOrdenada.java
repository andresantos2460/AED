...
public class ListaDuplaOrdenada<T> implements ColecaoIteravelLinearOrdenada<T> {
    ...
    public ListaDuplaOrdenada<T> removerTodos(T elem) {
        ListaDuplaOrdenada removidos=new ListaDuplaOrdenada(this.criterio);

        No noARemover=getNoPorOrdem(elem);

        if(noARemover.compararElemento(elem)!=0){
            return removidos;
        }

        No ultimoARemover=noARemover;

        while(noARemover!=base && noARemover.compararElemento(elem)==0){
            No cor=noARemover;
            noARemover=noARemover.seguinte;

            cor.anterior.seguinte=cor.seguinte;
            cor.seguinte.anterior=cor.anterior;
            numeroElementos--;

            cor.seguinte=removidos.base.seguinte;
            cor.seguinte.anterior=cor;

            removidos.base.seguinte=cor;
            cor.anterior=removidos.base;
